...
const ManageGmailSales = () => {
    ...
    const handleCopyCredentials = (sub: GmailSubmission) => {
        if (!navigator.clipboard) return;
        const textToCopy = `Gmail: ${sub.gmailAddress}\nPassword: ${sub.password}`;
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
            <h2 className="text-2xl font-bold text-gray-800 mb-4">Manage Gmail Sales</h2>
            
            <div className="mb-6 p-6 bg-slate-50 border border-gray-200 rounded-lg">
                <h3 className="text-xl font-bold text-gray-800 mb-2">জি-মেইল বিক্রির মূল্য নির্ধারণ</h3>
                <p className="text-gray-600">
                    প্রতিটি জি-মেইল অ্যাপ্রুভ করার জন্য ডিফল্ট মূল্য <span className="font-bold">৳{appSettings.gmailSellPrice}</span> সেট করা আছে।
                    <br />
                    আপনি চাইলে প্রতিটি সাবমিশনের জন্য এই মূল্য পরিবর্তন করে অ্যাপ্রুভ করতে পারেন।
                </p>
            </div>
                            ...
                            <tr key={sub.id} className="border-b hover:bg-slate-50">
                                ...
                                <td className="px-4 py-3">
                                    <div className="flex items-start justify-between">
                                        ...
                                        <button
                                            ...
                                            className="ml-2 px-2 py-1 text-xs font-semibold text-cyan-700 bg-cyan-100 rounded-md hover:bg-cyan-200 transition"
                                            aria-label="Copy Gmail and Password"
                                        >
                                ...
                                <td className="px-4 py-3">
                                    <div className="flex flex-col space-y-1">
                                        <span className={`px-2 py-1 text-xs font-medium rounded-full capitalize self-start ${
                                        ...
                                        {sub.status === 'approved' && sub.price && (
                                            <p className="text-xs font-semibold text-green-700 bg-green-100 px-2 py-1 rounded-md inline-block">
                                                Price: ৳{sub.price.toFixed(2)}
                                            </p>
                                        )}
                                        {sub.status === 'rejected' && sub.rejectionReason && (
                                            <p className="text-xs text-red-600 max-w-xs break-words">
